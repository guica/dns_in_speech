...
class AttentionResUnet(object):
    ...
    def attention_block(self, x, gating, inter_shape, prev_attention=None):
        ...
        # Combina o sinal de atenção anterior, se fornecido
        if prev_attention is not None:
            prev_attention = layers.Conv2D(inter_shape, (1, 1), padding='same')(prev_attention)
            upsample_g = layers.add([upsample_g, prev_attention])
        ...
        return result_bn, upsample_psi  # Retorna também o sinal de atenção gerado
    ...
    def get_model(self, dropout_rate=0.1, batch_norm=True):
        ...
        att_16, prev_attention = self.attention_block(conv_16, gating_16, 8*self.FILTER_NUM, prev_attention)
        ...
        att_32, prev_attention = self.attention_block(conv_32, gating_32, 4*self.FILTER_NUM, prev_attention)
        ...
        att_64, prev_attention = self.attention_block(conv_64, gating_64, 2*self.FILTER_NUM, prev_attention)
        ...
        att_128, prev_attention = self.attention_block(conv_128, gating_128, self.FILTER_NUM, prev_attention)
