...
class AttentionResUnet(object):
    ...
    def get_model(self, dropout_rate=0.1, batch_norm=True):
        ...
        pool_4 = layers.AveragePooling2D(pool_size=(2,2))(conv_8)
        # DownRes 6, convolution only
        conv_4 = self.double_conv_layer(pool_4, self.FILTER_SIZE, 32*self.FILTER_NUM, dropout_rate, batch_norm)

        # ConvLSTM layer for capturing temporal dependencies in the encoded feature space.
        # convlstm = ConvLSTM1D(128, 64, padding='same', return_sequences=True)(conv_4)

        # Upsampling layers
        # UpRes 7, attention gated concatenation + upsampling + double residual convolution
        gating_8 = self.gating_signal(conv_4, 16*self.FILTER_NUM, batch_norm)
        att_8 = self.attention_block(conv_8, gating_8, 16*self.FILTER_NUM)
        up_8 = layers.UpSampling2D(size=(self.UP_SAMP_SIZE, self.UP_SAMP_SIZE), data_format="channels_last")(conv_4)
        up_8 = layers.concatenate([up_8, att_8], axis=axis)
        up_conv_8 = self.double_conv_layer(up_8, self.FILTER_SIZE, 16*self.FILTER_NUM, dropout_rate, batch_norm)
        # UpRes 8
        gating_16 = self.gating_signal(up_conv_8, 8*self.FILTER_NUM, batch_norm)
        att_16 = self.attention_block(conv_16, gating_16, 8*self.FILTER_NUM)
        up_16 = layers.UpSampling2D(size=(self.UP_SAMP_SIZE, self.UP_SAMP_SIZE), data_format="channels_last")(up_conv_8)
